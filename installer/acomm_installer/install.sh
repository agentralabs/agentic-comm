#!/bin/bash
# AgenticComm -- one-liner install script
...
#
# What it does:
#   1. Downloads agentic-comm-mcp binary to ~/.local/bin/
...
# -- Constants ----------------------------------------------------------------
REPO="agentralabs/agentic-comm"
BINARY_NAME="agentic-comm-mcp"
SERVER_KEY="agentic-comm"
...
ensure_rust() {
    ...
    if [ "$DRY_RUN" = true ]; then
        echo "  [dry-run] Would run: curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y"
        return
    fi
...
download_binary() {
    ...
    local asset_name="agentic-comm-${version_num}-${platform}.tar.gz"
    ...
    # Copy both binaries (acomm CLI + MCP server)
    cp "${tmpdir}"/agentic-comm-*/acomm "${INSTALL_DIR}/acomm" 2>/dev/null || true
    cp "${tmpdir}"/agentic-comm-*/${BINARY_NAME} "${INSTALL_DIR}/${BINARY_NAME}"
    chmod +x "${INSTALL_DIR}/acomm" 2>/dev/null || true
...
install_from_source() {
    ...
    if [ "$DRY_RUN" = true ]; then
        echo "  [dry-run] Would run: cargo install --git ${git_url} ${source_ref_text}--locked --force agentic-comm-cli"
        echo "  [dry-run] Would run: cargo install --git ${git_url} ${source_ref_text}--locked --force agentic-comm-mcp"
        echo "  [dry-run] Would copy from ${cargo_bin}/(acomm,${BINARY_NAME}) to ${INSTALL_DIR}/"
        return
    fi
    ...
    if [ -n "${VERSION:-}" ] && [ "${VERSION}" != "latest" ]; then
        run_with_progress 45 68 "Installing agentic-comm-cli" \
            cargo install --git "${git_url}" --tag "${VERSION}" --locked --force agentic-comm-cli
        run_with_progress 68 85 "Installing agentic-comm-mcp" \
            cargo install --git "${git_url}" --tag "${VERSION}" --locked --force agentic-comm-mcp
    else
        run_with_progress 45 68 "Installing agentic-comm-cli" \
            cargo install --git "${git_url}" --locked --force agentic-comm-cli
        run_with_progress 68 85 "Installing agentic-comm-mcp" \
            cargo install --git "${git_url}" --locked --force agentic-comm-mcp
    fi

    mkdir -p "${INSTALL_DIR}"
    cp "${cargo_bin}/acomm" "${INSTALL_DIR}/acomm"
    cp "${cargo_bin}/${BINARY_NAME}" "${INSTALL_DIR}/${BINARY_NAME}"
    chmod +x "${INSTALL_DIR}/acomm" "${INSTALL_DIR}/${BINARY_NAME}"
    echo "  Installed from source to ${INSTALL_DIR}/acomm and ${INSTALL_DIR}/${BINARY_NAME}"
}
...
find_commfile() {
    ...
    for candidate in \
        "\${ACOMM_FILE:-}" \
        "\${AGENTRA_ACOMM_PATH:-}" \
        "\${repo_root}/.agentra/\${repo_slug}.acomm" \
        "\${repo_root}/.comm.acomm" \
        "\$PWD/.comm.acomm" \
        "\$PWD/comm.acomm" \
        "\$HOME/.comm.acomm" \
        "\$HOME/.agentra/comm/default.acomm"; do
    ...
    printf '%s' "\${repo_root}/.agentra/\${repo_slug}.acomm"
}
...
if [ "\$has_commfile" -eq 0 ]; then
    commfile_path="\$(find_commfile || true)"
    if [ -n "\$commfile_path" ]; then
        args=(--file "\$commfile_path" "\${args[@]}")
    fi
fi
...
print_post_install_next_steps() {
    ...
    if [ "$PROFILE" = "server" ]; then
        ...
        echo "  3. If artifacts were created on another machine, sync .acomm/.amem/.acb/.avis files to this server." >&3
        echo "  4. Start MCP with auth, connect clients, then restart clients." >&3
        echo "  5. Optional feedback: open https://github.com/agentralabs/agentic-comm/issues" >&3
    else
        echo "  2. Restart your MCP client/system so it reloads MCP config." >&3
        echo "  3. ${SERVER_KEY} now auto-detects local .acomm files at startup when available." >&3
...
main() {
    ...
    finish_progress
    echo "Install complete: AgenticComm (${PROFILE})" >&3
    echo "" >&3
    echo "Done! Comm file defaults to .agentra/<project>.acomm" >&3
