...
CORE_VERSION="$(grep -m1 '^version\s*=\s*"' crates/agentic-comm/Cargo.toml | sed -E 's/.*"([^"]+)".*/\1/')"
MCP_VERSION="$(grep -m1 '^version\s*=\s*"' crates/agentic-comm-mcp/Cargo.toml | sed -E 's/.*"([^"]+)".*/\1/')"
...
AgenticComm v${CORE_VERSION} (with MCP v${MCP_VERSION}) delivers production-grade agent-to-agent communication with channels, pub/sub, and message routing.

## Business Impact

This release enables reliable inter-agent communication for teams building multi-agent systems, reducing coordination overhead and improving message delivery guarantees.

## Rollout Guidance

Publish core first, validate availability on crates.io, then publish MCP and verify MCP client registration in staging before broad rollout.

## Source Links

- https://github.com/agentralabs/agentic-comm/compare/v${CORE_VERSION}...HEAD
EOF
...
echo "=== Publishing AgenticComm paired crates to crates.io ==="
...
echo "Dry run: agentic-comm"
cd crates/agentic-comm
cargo publish --dry-run
cd ../..

echo "Preflight: agentic-comm-mcp (build + lint path)"
cargo check -p agentic-comm-mcp
echo "Note: skipping MCP crates.io dry-run until the new core crate version is published."

echo ""
if [ "${DO_PUBLISH}" = true ]; then
  echo "Publishing core crate..."
  (cd crates/agentic-comm && cargo publish)
  echo "Waiting for crates.io propagation..."
  sleep 45
  echo "Publishing MCP crate..."
  (cd crates/agentic-comm-mcp && cargo publish)
  ...
  gh release create "v${CORE_VERSION}" \
    --title "AgenticComm v${CORE_VERSION}" \
