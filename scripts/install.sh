...
install_from_source() {
    ...
    if [ "$DRY_RUN" = true ]; then
        echo "  [dry-run] Would run: cargo install --git ${git_url} ${source_ref_text}--locked --force agentic-comm"
    ...
    if [ -n "${VERSION:-}" ] && [ "${VERSION}" != "latest" ]; then
        run_with_progress 45 68 "Installing agentic-comm" \
            cargo install --git "${git_url}" --tag "${VERSION}" --locked --force agentic-comm
        run_with_progress 68 85 "Installing agentic-comm-mcp" \
            cargo install --git "${git_url}" --tag "${VERSION}" --locked --force agentic-comm-mcp
    else
        run_with_progress 45 68 "Installing agentic-comm" \
            cargo install --git "${git_url}" --locked --force agentic-comm
        run_with_progress 68 85 "Installing agentic-comm-mcp" \
...
print_post_install_next_steps() {
    ...
    if [ "$PROFILE" = "server" ]; then
        ...
        echo "  3. If artifacts were created on another machine, sync .acomm files to this server." >&3
        ...
        echo "  3. ${SERVER_KEY} is now ready for agent-to-agent communication." >&3
...
main() {
    ...
    echo "" >&3
    echo "Done! Communication store defaults to .acomm files in your project." >&3
...
# Canonical guardrail strings (do not remove):
# MCP client summary:
